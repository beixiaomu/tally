...
<body>
			...
			<table class="table table-bordered">
				<caption>
					<button type="button" class="btn btn-default">
						...
						<a href="${ctx}/fileUpload/addFileUploadUI" target="right">添加</a>
					</button>
				...
				<tbody>
					<c:forEach items="${fileUploadList }" var="c">
						<tr>
							...
							<td>${c.tFileSize }</td>
							<td>${c.tFileType }</td>
							<td>${c.tFileDescribe }</td>
							<td>${c.tFileCount }</td>
							<td>${c.tCreateBy }</td>
							<td><fmt:formatDate value="${c.tCreateDate }"
									pattern="yyyy-MM-dd HH:mm:ss" /></td>
							<td><a class="btn bg-primary"
								href="${ctx}/fileUpload/addFileUploadUI?id=${c.id }" target="right">编辑</a>

								<button class="btn btn-danger" onclick="del(${c.id })">删除</button>
								<button class="btn btn-danger" onclick="download(${c.id })">下载</button>
							</td>
	...
	<script>
		...
		function del(id) {
			...
			$
					.ajax({
						url : "${pageContext.request.contextPath}/fileUpload/delete?id="
								+ id,
						type : "GET",
						dataType : "json",
		...
		function download(id) {
			...
			$
					.ajax({
						url : "${pageContext.request.contextPath}/fileUpload/download?id="
								+ id,
						type : "GET",
						dataType : "json",
						success : function(data) {
