...
<body>
		...
		<table class="table table-bordered">
			<caption>
				...
				<a id="export" class="btn btn-default"
					href="${pageContext.request.contextPath}/card/export"> <span
			...
			<tbody>
				<c:forEach items="${cList }" var="c">
					<tr>
						...
						<td><c:if test="${c.cardtype == 1}">学生卡</c:if> <c:if
								test="${c.cardtype == 2}">教师卡</c:if> <c:if
								test="${c.cardtype == 3}">未知卡</c:if></td>
						<td><c:if test="${c.cardstate == 1}">已激活</c:if> <c:if
								test="${c.cardstate == 2}">已挂失</c:if> <c:if
								test="${c.cardstate == 3}">已注销</c:if> <c:if
								test="${c.cardstate == 0}">未激活</c:if></td>
						<td>${c.cardmoney }</td>
						<td><fmt:formatDate value="${c.cardreleasedate}" pattern="yyyy-MM-dd HH:mm:ss" />
						</td>
						<td><fmt:formatDate value="${c.cardexpiredate}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
						<td>${c.cardserialnum }</td>
						<td>
							<button class="btn bg-primary" onclick="edit(${c.cardno })"
								data-toggle="modal" data-target="#studentModal"
								data-whatever="编辑">编辑</button>
							<button class="btn btn-danger" onclick="del(${c.cardno })">删除</button>
						</td>
	...
	<div class="modal fade" id="importModal" tabindex="-1" role="dialog"
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form action="${pageContext.request.contextPath}/card/import" method="post" id="importForm"
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
					<button type="button" class="btn btn-primary"
						onclick="importStudent()">确定</button>
				</div>
			</div>
		</div>
	</div>
     ...
     * @param id
     */
    function edit(cardno) {
        isUpdate = true;
        $.ajax({
            url: "${pageContext.request.contextPath}/card/select/" + cardno,
            type: "GET",
            dataType: "json",
            success: function (r) {
                if (r.status == 200) {
                    $("#cardno").val(r.data.cardno);
                    $("#hideenId").val(r.data.cardno);
                    $("#cardstate").val(r.data.cardstate);
                    $("#cardmoney").val(r.data.cardmoney);
                } else {
                    alert("加载数据错误");
                }
            },
    ...
    function saveOrUpdate() {
        var url = "${pageContext.request.contextPath}/card/add";
        if (isUpdate) {
            url = "${pageContext.request.contextPath}/card/update/" + $("#hideenId").val();
        }
        $.ajax({
            url: url,
            type: "POST",
    ...
    function del(cardno) {
        $.ajax({
            url: "${pageContext.request.contextPath}/card/delete/" + cardno,
            type: "GET",
            dataType: "json",
