...
<script>
    ...
    $(function () {
        ...
        function initTable() {
            $('#table').bootstrapTable({
                url: '${pageContext.request.contextPath}/slotcard/loadListData',
                dataType: "json",
                method: 'get',                      //请求方式（*）
                ...
                columns: [
                    ...
                    
                    {
                        ...
                        formatter: function (value, row, index) {
                            var e = '<button class="btn bg-primary" onclick="edit(' + row.slotno + ')" data-toggle="modal" data-target="#studentModal" data-whatever="编辑">编辑</button> ';
                            var d = '<button class="btn btn-danger" onclick="del(' + row.slotno + ')">删除</button> ';
                            return e + d;
                        }
                    }
                ]
    ...
    function edit(slotno) {
        isUpdate = true;
        $.ajax({
            url: "${pageContext.request.contextPath}/slotcard/select/" + slotno,
            type: "GET",
            dataType: "json",
            success: function (r) {
                if (r.status == 200) {
    ...
    function del(slotno) {
        $.ajax({
            url: "${pageContext.request.contextPath}/slotcard/delete/" + slotno,
            type: "GET",
            dataType: "json",
            success: function (data) {
                if (data.status == 200) {
